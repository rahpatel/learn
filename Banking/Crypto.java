...
public class Crypto {
    ...
    public synchronized  byte[] encryptRSAPrivateKey(Serializable obj, PrivateKey kPriv) 
    throws KeyException, IOException
    {
    try {
        Cipher RSACipher = Cipher.getInstance("RSA/NONE/PKCS1Padding","BC");
        return encrypt(RSACipher, obj, kPriv);
    } catch (Exception e) {
        e.printStackTrace();
        System.exit(1);
    }
    ...
    public synchronized  Object decryptRSAPublicKey(byte[] msgE, PublicKey  kPub)
    throws KeyException, IOException
    {
    try {
        Cipher RSACipher = Cipher.getInstance("RSA/NONE/PKCS1Padding","BC");
        return decrypt(RSACipher, msgE, kPub);
    } catch (Exception e) {
        e.printStackTrace();
        System.exit(1);
    }
